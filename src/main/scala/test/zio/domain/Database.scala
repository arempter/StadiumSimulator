...
object Database {
  ...
  trait Service {
    ...
    def exists(ticket: GameTicket): STM[Nothing, Boolean]
    def insert(ticket: GameTicket): STM[Nothing, Unit]
    def count(): UIO[Int]
  ...
  def insert(ticket: GameTicket): ZSTM[Database, Nothing, Unit] = ZSTM.accessM(_.get.insert(ticket))
